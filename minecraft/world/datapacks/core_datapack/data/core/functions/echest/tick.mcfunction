...
execute if entity @p[scores={ender_chest=1}] as @a[scores={ender_chest=1}] if entity @s[nbt={recipeBook:{recioes:["minecraft:ender_chest"]}}] run tag @s add drop_ender_chest

execute if entity @p[scores={ender_chest=1}] as @a[scores={ender_chest=1}] unless entity @s[nbt={recipeBook:{recioes:["minecraft:ender_chest"]}}] run tellraw @s {"text":"You must have crafted atleast one ender chest to use this feature","color":"yellow"}
