...
execute if entity @e[tag=spawn,type=armor_stand] as @e[tag=spawn,type=armor_stand] at @a[distance=..16] unless entity @e[tag=guard,distance=0,type=slime] positioned ~ 256 ~ run tp @e[tag=guard,distance=0,type=slime,limit=1] @s
