...
execute if entity @p[team=!away,scores={difficulty=5}] as @a[team=!away,scores={difficulty=5}] run tellraw @a[scores={difficulty_timer=1}] [{"selector":"@s"}," would like to set the difficulty to Normal.","\n",{"color":"light_purple","text":"To change difficulty all players must click here to accept.","clickEvent":{"action":"run_command","value":"/trigger difficulty set 4"}}]
