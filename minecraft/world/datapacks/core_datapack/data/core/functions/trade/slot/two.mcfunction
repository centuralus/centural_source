...
execute if entity @p[tag=send_offer_two,scores={trade=-1}] if entity @s[tag=received_offer_two,scores={trade=2}] run item entity @p[tag=received_offer_two,scores={trade=-1}] weapon.offhand copy entity @e[tag=slot_two,tag=receive,limit=1] weapon.mainhand
