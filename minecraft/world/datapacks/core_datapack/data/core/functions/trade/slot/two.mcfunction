...
execute unless entity @s[tag=send_offer_two] unless entity @e[tag=send,tag=slot_two,type=armor_stand] at @s positioned ^ ^1 ^ positioned ~ ~1 ~ run summon armor_stand ~ ~ ~ {Invulnerable:1b,Small:1b,Invisible:1b,NoGravity:1b,NoBasePlate:1b,PersistenceRequired:1b,Health:1f,DisabledSlots:4144703,HandItems:[{},{}],Tags:["keep","send","slot","slot_two"]}
...
execute if entity @s[tag=send_offer_two] unless entity @p[tag=received_offer_two] if entity @p[tag=receive_offer_two] run tellraw @p[tag=receive_offer_two] [{"selector":"@s"},{"text":" would like to trade ","color":"yellow"},{"nbt":"HandItems[0]","entity":"@e[tag=slot_two,tag=send]"},"\n",{"color":"light_purple","text":"To offer an item hold the item in your left hand then Click Here.","clickEvent":{"action":"run_command","value":"/trigger trade set 2"}}]
