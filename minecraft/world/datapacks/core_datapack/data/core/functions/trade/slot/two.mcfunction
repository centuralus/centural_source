...
execute if entity @s[tag=receive_offer_two,tag=!received_offer_two] unless entity @e[tag=receive,tag=slot_two,type=armor_stand] at @s run summon armor_stand ~ ~ ~ {Invulnerable:1b,Small:1b,Invisible:1b,NoGravity:1b,NoBasePlate:1b,PersistenceRequired:1b,Health:1f,DisabledSlots:4144703,HandItems:[{},{}],HandDropChances:[1.000F,0.085F],Tags:["keep","receive","slot","slot_two"]}
