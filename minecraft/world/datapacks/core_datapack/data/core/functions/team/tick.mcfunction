...
execute if entity @p[team=] as @a[team=] unless entity @s[scores={walk_one_cm=0..}] run tellraw @s ["If you are missing a Structure please visit pre.centural.us and get with an Admin to help transfer you Structure! (This proccess will be easier later this year with an Item Transfer for Strcutres thanks to Cloud Wolf)"]

execute if entity @p[team=] as @a[team=] unless entity @s[scores={walk_one_cm=0..}] run tellraw @s ["To join our ",{"text":"Discord Server Click Here","clickEvent":{"action":"open_url","value":"https://discord.com/invite/YV4fEuE9fm"}say ,"hoverEvent":{"action":"show_text","contents":[{"text":"Invite Code ~ YV4fEuE9fm"}]},"color":"red"}]
